...
type PageMessage = {
  type: 'connectToMCPRelay' | 'disconnectFromMCPRelay';
  mcpRelayUrl?: string;
};

export class TabShareExtension {
    ...
    switch (message.type) {
      case 'connectToMCPRelay':
        ...
        if (sender.tab?.id) {
          ...
          // Message is from popup, get the active tab
          chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            if (tabs.length > 0) {
              this._connectTab(tabs[0].id!, message.mcpRelayUrl!).then(
  ...
  private async _disconnect(): Promise<void> {
    if (this._activeConnection) {
      this._activeConnection.close('Disconnected by user');
      this._activeConnection = undefined;
    }
    if (this._connectedTabId) {
      await this._updateBadge(this._connectedTabId, { text: '', color: null });
  ...
  private async _onTabUpdated(tabId: number, changeInfo: chrome.tabs.OnUpdatedInfo, tab: chrome.tabs.Tab): Promise<void> {
