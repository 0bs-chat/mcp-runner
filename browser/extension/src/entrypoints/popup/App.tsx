...
export default function App() {
  ...
  const loadConnectionStatus = async () => {
    try {
      const status = await sendMessage('WsConnection', 'getStatus') as ConnectionStatus['status'];
  ...
  const loadTabs = async () => {
    try {
      const tabsData = await sendMessage('TabManager', 'listTabs') as [string, number][];
      ...
      {activeTab === 'tabs' && (
        <div className="space-y-4">
          <div>
            ...
            {tabs.length === 0 ? (
              <p className="text-sm text-muted-foreground italic">No tabs created yet</p>
            ) : (
                ...
                {tabs.map((tab) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">ID: {tab.id}</p>
                    </div>
